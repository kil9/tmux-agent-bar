...
코드, 주석, 설정 예시, 문서(README/AGENTS/PLAN) 어디에도 아래 항목을 포함하지 않는다:

- **내부 호스트명·IP·도메인**: 내부 서버 주소, 인트라넷 URL, 내부 DNS 이름
- **인증정보**: 내부 API 키, 내부 OAuth 토큰, 내부 DB 접속 정보
- **조직 식별자**: 사내 팀명·코드명·프로젝트명, 사원번호, 내부 시스템 명칭
- **로컬 경로**: 회사 서버의 홈 디렉토리 경로나 사내 파일시스템 구조

### 코드/문서 작성 시 체크리스트

1. 예시 호스트명은 `example.com`, `localhost`, `<your-host>` 등 중립적 값을 사용한다.
2. 예시 경로는 `~/`, `$HOME/`, `<path>` 형태로 표기한다.
3. 커밋 전, diff를 훑어 내부 정보가 포함되지 않았는지 확인한다.
4. 스크린샷·로그 첨부 시 사내 정보가 노출되지 않도록 마스킹한다.
...
처리 중(🤖) 표시는 실행 모델에 따라 전환된다: ✨ Fable · 🌀 Opus · 🤖 Sonnet · 🖥️ Haiku (pane `.meta`의 모델 ID 기준, 불명 시 🤖). 상태 우선순위·의미는 불변이고 표기만 바뀐다(`modelEmoji`가 `shortModelName` tier에 위임).
...
- 상태 파일 키는 tmux `#S\t#I\t#P`(탭 구분)로 session/window/pane을 직접 받아 조립(`tmuxPaneParts`→`stateKey`)하므로, 세션명에 `_`가 있어도 정리 로직(`cleanStaleFiles`)이 어긋나지 않는다. 파일명 스킴 자체는 불변
...
`thinking` 상태는 TTL을 두어, Stop hook이 발사되지 않은 채 종료된 죽은 세션에서 🤖가 영구히 남는 것을 방지한다. 추가로 `runStatus`가 사라진 window/세션의 잔여 파일을 주기적으로(최대 5분 간격, `.gc` 마커로 throttle) GC 한다. 원자적 쓰기가 크래시로 남긴 `.tmp-*` 잔재는 1시간 이상 지난 것만 이 GC가 함께 회수한다(`orphanTmpMaxAge`).

**macOS 등 /proc 폴백**: claude-right의 stale meta 가드(살아있는 claude 없으면 meta 삭제)와 ⏳ 판정은 프로세스 트리 탐색(`/proc/<pid>/task/<pid>/children`)에 의존한다. macOS는 /proc이 없고 일부 Linux 커널은 `CONFIG_PROC_CHILDREN`이 없어 이 파일을 못 읽는다. 그 경우 `procTreeAvailable()`이 false가 되어 stale meta 가드를 건너뛰고 meta를 신뢰하므로(ctx%/model이 계속 표시됨), 트리 탐색 불가를 "살아있는 claude 없음"으로 오판해 meta를 지우는 회귀가 없다. ⏳는 이런 시스템에서 자연히 미표시된다.
