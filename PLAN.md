...
### 7. 문서·잔재 소소한 정리 (한 커밋으로 묶음) ✅ 완료 (2026-07-02)
...
### 8. [폐기] ctx% 분모 머신 로컬 설정 (kil9conf 연계) — claude-right 기능 제거로 불필요
...
자동 실행 보류 (2026-07-02): `#()` 의 `set-environment -g` 상속 여부를 별도 소켓 임시 서버로 실측
시도했으나, 이 tmux 에서는 `display-message -p` 로 `#()` 가 즉석 평가되지 않아(빈 값 반환) 세션에서
확정 불가 - 실제 status 렌더 + 눈 확인이 필요하다. 또한 영속화 핵심인 kil9conf(`rc/tmux.conf`) 수정·
커밋은 타 저장소 컨벤션(AGENTS/README 검토)이라 이 세션 범위 밖이고, `~/.tmux.local.conf` 단독 생성은
`source-file -q` 줄 없이는 무효, ctx% 완화 확인도 시각이 필요하다. 리드 전담으로 남기고 사용자 후속
처리 권장(권장 순서: kil9conf `rc/tmux.conf` 말미에 `source-file -q ~/.tmux.local.conf` 추가 → 이 머신에
`~/.tmux.local.conf` 생성 → tmux 재기동 or `set-environment -g` 후 status ctx% 눈 확인 → 상속 안 되면
셸 export 로 대체 → auto-memory 기록).

### 9. 잔존 agent worktree 정리 ✅ 완료 (2026-07-02)
...
- 2026-07-02: 사용자 요청으로 **claude-right(status-right ctx%+model) 기능 제거**. `runClaudeRight`·`procTreeAvailable(In)`·`contextLimit`·`tmuxPanePID`·`defaultContextTokens`·워치독/디스패치/usage 분기·install status-right 템플릿·`TestProcTreeAvailableIn`·`TestContextLimit`·관련 문서(README/AGENTS)를 삭제. 공유 함수(`findClaudeDescendants`=⏳ 판정, `readMeta`/`writeMeta`, `shortModelName`=🤖 모델 이모지)는 유지. 이로써 태스크 4(claude-right macOS 회귀 수정)와 태스크 8(ctx% 머신 로컬)은 무효화. kil9conf `rc/tmux.conf`의 status-right에서도 claude-right 토큰 제거(날짜/시간 유지).
- 2026-07-02: 태스크 4·5·6·7·10 자동 구현 완료(각 별도 커밋). 태스크 9로 잔존 worktree `.claude/worktrees/agent-acac9dfa`(브랜치 `worktree-agent-acac9dfa`, tip `ffba347008d164af47fdee534ca54c8062d686df`) 삭제 — 초기 구축(Phase 1-4) 산물로 내용은 현재 main에 모두 반영·대체됨. 브랜치 미커밋 변경(4/7자 초기 실험, main에 흡수됨)은 삭제 전 patch로 백업 후 `--force` 제거. tip은 reflog로 복구 가능.
